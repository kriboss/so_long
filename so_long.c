...
/*   Updated: 2025/03/07 12:57:55 by kbossio          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (i == 100000 || i == 200000)
	{
		while (j >= 0)
		{
			mlx_put_image_to_window(l->mlx, l->wnd,
				l->img->bg, l->en->x[j] * 100, l->en->y[j] * 100);
			mlx_put_image_to_window(l->mlx, l->wnd,
				l->img->tile, l->en->x[j] * 100, l->en->y[j] * 100);
			mlx_put_image_to_window(l->mlx, l->wnd,
				l->img->en[i / MF], l->en->x[j] * 100, l->en->y[j] * 100);
			j--;
		}
	}
	if (l->img->frame == MF)
